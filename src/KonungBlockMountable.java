...
public class KonungBlockMountable extends Block
{
	
	//This constructor just pass thing on.
	public KonungBlockMountable(int x, Material material)
	{
		super(x, material);
	}
	
	//This constructor just pass thing on.
	public KonungBlockMountable(int x, int y, Material material)
	{
		super(x, y, material);
	}
	...
	public static boolean blockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, float x, float y, float z, int north, int south, int east, int west)
    {
		if (!world.isRemote)
		{
			...
			List<KonungEntityMountableBlock> list = world.getEntitiesWithinAABB(KonungEntityMountableBlock.class, AxisAlignedBB.getBoundingBox(i, j, k, i + 1.0D, j + 1.0D, k + 1.0D).expand(1D, 1D, 1D));
	    	for (KonungEntityMountableBlock entitytocheck : list)
	    	{
			...
			KonungEntityMountableBlock nemb = new KonungEntityMountableBlock(world, entityplayer, i, j, k, mountingX, mountingY, mountingZ); 
